...
var maxSurgePercent = decimal.NewFromInt(150)
...
func (s *service) calculateSurge(
	...
	total := timePercent.Add(demandPercent).Add(weatherPercent)
	if total.GreaterThan(maxSurgePercent) {
	...
	return SurgeBreakdown{
		...
		TotalPercent:     total,
		Multiplier:       decimal.NewFromInt(1).Add(total.Div(decimal.NewFromInt(100))),
	}
}
...
func timeOfDaySurgePercent(rules []SurgeTimeRule, now time.Time) decimal.Decimal {
	...
	for _, rule := range rules {
		...
		if rule.SurgePercent.GreaterThan(highest) {
...
func (s *service) demandSurgePercent(ctx context.Context, pickupLat, pickupLng float64) decimal.Decimal {
