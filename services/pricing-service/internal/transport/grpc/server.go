...
func (s *Server) RegisterPricingService(
	handler pricingv1.PricingServiceServer,
) {
	if handler == nil {
		panic("pricing service handler is required")
	}

	pricingv1.RegisterPricingServiceServer(
		s.grpcServer,
		handler,
	)
}
