...
func TestIdentifierLinkCompletionStoreAllowsOnlyOneConcurrentChallengeReplay(
	t *testing.T,
) {
	ctx, pool, store, challengeRepository :=
		newIdentifierLinkCompletionIntegrationTest(t)
	...
	identifier := auth.Identifier{
		Type:  auth.IdentifierTypeEmail,
		Value: "same-challenge-race@example.com",
	}

	const challengeID = "otp_ch_link_completion_same_challenge_race"
	...
	challenge := auth.OTPChallenge{
		...
		CodeHash:         "same-challenge-race-hash",
		ExpiresAt: time.Now().
			UTC().
			Add(5 * time.Minute),
	}
	...
	if err := challengeRepository.Create(
		...
		t.Fatalf(
			"create identifier link challenge: %v",
			err,
		)
	}
	...
	if err != nil {
		t.Fatalf(
			"count linked identifier rows: %v",
			err,
		)
	}
	...
	if identifierCount != 1 {
		t.Fatalf(
			"linked identifier rows = %d, want 1",
			identifierCount,
		)
	}
