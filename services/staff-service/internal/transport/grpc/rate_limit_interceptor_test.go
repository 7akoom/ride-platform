...
func TestRateLimitInterceptor_AllowsUpToBurstThenRejects(t *testing.T) {
	...
	ctx := contextForIdentity("staff-1")
	info := &googlegrpc.UnaryServerInfo{FullMethod: "/ride.staff.v1.StaffService/GetMyStaffProfile"}
...
func TestRateLimitInterceptor_TracksSeparateBucketsPerPrincipal(t *testing.T) {
	...
	// Exhaust staff-1's single-token bucket.
	if _, err := interceptor(contextForIdentity("staff-1"), nil, info, noopHandler); err != nil {
		t.Fatalf("staff-1 first call: unexpected error %v", err)
	}
	if _, err := interceptor(contextForIdentity("staff-1"), nil, info, noopHandler); !isResourceExhausted(err) {
		t.Fatalf("staff-1 second call: expected ResourceExhausted, got %v", err)
	}

	// A different caller must not be affected by staff-1's exhausted bucket.
	if _, err := interceptor(contextForIdentity("staff-2"), nil, info, noopHandler); err != nil {
		t.Fatalf("staff-2 first call: expected a fresh bucket, got %v", err)
	}
}
...
func TestRateLimitInterceptor_ExemptsHealthCheck(t *testing.T) {
	...
	ordinaryInfo := &googlegrpc.UnaryServerInfo{FullMethod: "/ride.staff.v1.StaffService/GetMyStaffProfile"}
