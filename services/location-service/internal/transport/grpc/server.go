...
func (s *Server) RegisterLocationService(
	handler locationv1.LocationServiceServer,
) {
	if handler == nil {
		panic("location service handler is required")
	}

	locationv1.RegisterLocationServiceServer(
		s.grpcServer,
		handler,
	)
}
