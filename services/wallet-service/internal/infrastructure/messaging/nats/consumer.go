...
func SubscribeDurable(
	...
	consumer, err := js.CreateOrUpdateConsumer(
		...
		jetstream.ConsumerConfig{
			...
			// history: replaying every old fare.calculated would settle
			// (and move real money for) trips that finished long before
			// this consumer existed. Once created, the durable resumes
			// exactly where it left off, so nothing is missed across
			// restarts.
