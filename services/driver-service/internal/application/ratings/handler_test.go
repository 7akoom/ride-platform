...
func TestHandle_AppliesTheRatingsGivenByTheOtherSide(t *testing.T) {
	...
	err := newHandler("rider", applier).Handle(context.Background(), ratings.SubjectTripRated,
		event("rider", "4", ratingID, rateeID))
	if err != nil {
...
func TestHandle_IgnoresWhatIsNotForThisService(t *testing.T) {
	cases := []struct {
		...
		{"another subject", "trip.completed", event("rider", "4", ratingID, rateeID)},
		{"the rating given by this service's own side", ratings.SubjectTripRated, event("driver", "4", ratingID, rateeID)},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := newHandler("rider", applier).Handle(context.Background(), tc.subject, tc.data); err != nil {
				t.Fatalf("Handle: %v", err)
			}
...
func TestHandle_DropsWhatCanNeverBeApplied(t *testing.T) {
	cases := []struct {
		...
		{"zero stars", event("rider", "0", ratingID, rateeID)},
		{"six stars", event("rider", "6", ratingID, rateeID)},
		{"stars that are not a number", event("rider", "five", ratingID, rateeID)},
		{"a rating id that is not a UUID", event("rider", "4", "nope", rateeID)},
		{"a rated profile id that is not a UUID", event("rider", "4", ratingID, "nope")},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := newHandler("rider", applier).Handle(context.Background(), ratings.SubjectTripRated, tc.data); err != nil {
