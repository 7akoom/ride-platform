...
func SubscribeDurable(
	...
	consumer, err := js.CreateOrUpdateConsumer(
		...
		jetstream.ConsumerConfig{
			...
			// history: replaying every old event would apply changes that
			// were already dealt with long before this consumer existed.
			// Once created, the durable resumes exactly where it left off,
			// so nothing is missed across restarts.
