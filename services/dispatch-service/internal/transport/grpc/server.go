...
func (s *Server) RegisterDispatchService(
	handler dispatchv1.DispatchServiceServer,
) {
	if handler == nil {
		panic("dispatch service handler is required")
	}

	dispatchv1.RegisterDispatchServiceServer(
		s.grpcServer,
		handler,
	)
}
