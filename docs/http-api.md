...
| POST | `/v1/trips/{tripId}/shares` | rider of the trip | a link for people the rider trusts, only while the trip is requested, accepted or in progress (400 `FAILED_PRECONDITION` otherwise, and for a 6th live link): `token` (43 characters, returned only here; only its hash is stored), `url` (`TRIP_SHARE_URL_BASE` + token, empty when the app builds the URL itself), `createdAt`, `expiresAt` (`TRIP_SHARE_MAX_AGE`, 12 h) |
| POST | `/v1/trips/{tripId}/shares:stop` | rider of the trip | ends every link to the trip: `stopped`, how many |
| GET | `/v1/shared-trips/{token}` | **anyone with the link, no token** | `trip`: `status`, `pickup`, `dropoff`, the addresses, `stops`, `driverName` and `vehicle` once a driver has it, `driverLocation` and `driverLocationUpdatedAt` while accepted or in progress, the trip's times, and `linkExpiresAt`. Never the rider, the passenger, the price or a phone number. 404 for any link that does not work (unknown, stopped, expired, or its trip ended more than `TRIP_SHARE_AFTER_END`, 30 minutes, ago). Counted for the rate limit by address; poll about every 5 s |
